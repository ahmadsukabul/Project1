	            ...
	            <div class='col-md-12 col-xs-12 col-lg-12'>
					...
					<section class="card card-blue mb-3">
						...
						<div class="card-block">
							...
							<?PHP
							$total_masuk = $db->fetch("select sum(total_transfer) from topup where status=1 and created_at>='$d_start' and created_at<='$d_end'");
							$total_masuk = $total_masuk[0];
							$jml = $db->num_rows("select id from topup where status=1 and created_at>='$d_start' and created_at<='$d_end'");
							?>
							<div class="row" style='margin-bottom:0px'>
								<div class="col-md-6">
									<section class="widget widget-simple-sm">
										<div class="widget-simple-sm-statistic">
											<div class="number"><?PHP echo $app->idr($total_masuk); ?></div>
											<div class="caption color-green">Total Topup User</div>
										</div>
										<div class="widget-simple-sm-bottom statistic"><span class="fa fa-money color-green"></span> User topup ke sistem</div>
									</section><!--.widget-simple-sm-->
								</div>
								<div class="col-md-6">
									<section class="widget widget-simple-sm">
										<div class="widget-simple-sm-statistic">
											<div class="number"><?PHP echo $app->angka_id($jml); ?></div>
											<div class="caption color-blue">Jumlah Topup user</div>
										</div>
										<div class="widget-simple-sm-bottom statistic"><span class="fa fa-list color-blue"></span> Jumlah topup berhasil ke sistem</div>
									</section><!--.widget-simple-sm-->
								</div>
							</div>
							...
							foreach($metode as $mtd){
								...
								$jml_masuk = $db->num_rows("select id from topup where status=1 and topup_metode='$m_id' and created_at>='$d_start' and created_at<='$d_end'");
							?>
							
							<tr>
							  <td>#<?PHP echo $m_id; ?></td>
							  <td><?PHP echo "$m_nama_kategori - $m_nama_metode"; ?></td>
							  <td><?PHP echo $app->angka_id($jml_masuk); ?></td>
